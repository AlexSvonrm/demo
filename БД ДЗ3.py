...
with psycopg2.connect(database = "clients_db", user= "postgres", password= "postgres") as conn:
    create_db(conn)
    print("База создана")
    print("Добавлен клиент id: ", add_client(conn, "Иван", "Иванов", "dfg@.com"))
    ...
    print("Телефон добавлен клиенту id: ", add_phone(conn, 1, "1-111-11-11"))
    add_phone(conn, 2, "2-222-22-22")
    add_phone(conn, 3, "3-333-33-33")
    add_phone(conn, 4, "4-444-44-44")
    add_phone(conn, 5, "5-555-55-55")
    print("Изменены данные клиента id: ", change_client(conn, 2))
    print("Телефон удалён c номером: ", delete_phone(conn, 3, "3-333-33-33"))
    print("Клиент удалён с id: ", delete_client(conn, 4))
    print('Найденный клиент по имени:', find_client(conn, "Коля"))
